...
def get_predicted_volumes(pred):
    ...
    labels_dict = dict(zip(*np.unique(pred, return_counts=True)))
    
    volume_ant, volume_post = labels_dict[1], labels_dict[2] 
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                         f"Patient ID: {header.PatientID}\nHippocampal total volume: {inference['total']} mm^3\nAnterior volume: {inference['anterior']} mm^3\nPosterior volume: {inference['posterior']} mm^3\n", (255, 255, 255), font=main_font)
...
def get_series_for_inference(path):
    ...
    dicoms = list(chain(*[[pydicom.dcmread(os.path.join(pd, f)) for f in files] for pd, _, files in os.walk(path) if len(files) != 0]))
    ...
    # Answer: Series Description = "HippoCrop"
...
if __name__ == "__main__":
    ...
    # routed studies
    if len(sys.argv) != 2:
        print("You should supply one command line argument pointing to the routing folder. Exiting.") # TestVolumes/
        sys.exit()
