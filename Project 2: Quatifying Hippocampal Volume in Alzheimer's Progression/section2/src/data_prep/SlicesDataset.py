...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = self.data[image_idx]['image'][slice_idx]
        expanded_image = image[np.newaxis, :]
        
        label = self.data[image_idx]['seg'][slice_idx]
        expanded_label = label[np.newaxis, :]
        
        sample["image"] = torch.from_numpy(expanded_image)
        sample["seg"] = torch.from_numpy(expanded_label)
